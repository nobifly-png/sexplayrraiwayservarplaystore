...
const _handleClipNovaLink = async (ctx, session, shortCode, pendingThumb) => {
  ...
  try {
    ...
    if (thumbUrl) {
      await ctx.telegram.sendPhoto(chatId, thumbUrl, { caption, disable_web_page_preview: true }).catch(() =>
        ctx.telegram.sendMessage(chatId, caption, { disable_web_page_preview: true }).catch(() => {})
      );
    } else {
      await ctx.telegram.sendMessage(chatId, caption, { disable_web_page_preview: true }).catch(() => {});
    }
