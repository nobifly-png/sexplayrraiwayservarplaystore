...
class UploadService {
  ...
  async completeUpload(creatorId, videoId) {
    ...
    try {
      ...
      // ✨ AUTO-GENERATE LINK when video becomes READY
      let autoGeneratedLink = null;
      try {
        autoGeneratedLink = await linkService.createLink(creatorId, videoId, {});
        logger.info({ videoId, linkId: autoGeneratedLink._id, shortCode: autoGeneratedLink.shortCode }, 
          'Auto-generated share link for uploaded video');
      } catch (linkError) {
        // Don't fail the upload if link creation fails
        logger.error({ err: linkError, videoId }, 'Failed to auto-generate link for video');
      }

      return { video, link: autoGeneratedLink };
    } catch (error) {
